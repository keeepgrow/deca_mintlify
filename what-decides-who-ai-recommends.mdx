...
description: "Two new research papers predict that GEO optimization homogenizes content and that strategy value collapses as competitors adopt it. We tested both predictions against 14 weeks of on-page audit data across two boards: pattern adoption converged on both, but the ranking consequences were opposite — a five-flip #1 in Project Management, an untouched #1 in AI Image Generators."
...
In the last week of August, two research papers arrived at the same warning from different directions. [CHASE](https://arxiv.org/abs/2608.30466) (arXiv, August 31) simulated what happens when creators repeatedly optimize content against an LLM ranking signal. The content homogenizes, and the advantage each optimization step buys shrinks. [Beyond the Vacuum](https://arxiv.org/abs/2608.27631) (arXiv, August 27) formalized the reason. GEO strategies are usually evaluated as if no one else is optimizing, but as competitor adoption grows, the value of any given strategy changes.

Both papers are simulations. We track the real thing. Every week, DecaGEO audits the pages ChatGPT cites for each ranked brand against 16 on-page GEO patterns such as answer-first structure, question headings, FAQ markup, meta completeness, and crawler policies. That gives us 14 weeks of adoption history (May 31 – August 30, 2026) to test the papers' predictions against.
...
A pattern that 95% of your competitors already run can no longer be what separates you from them. In our data, this is Beyond the Vacuum's core claim playing out. The standard checklist now reads as an entry requirement.
...
The compressed board isn't just adopting the shared playbook faster. It's also generating far more one-off experiments beyond it, and none of those experiments has stabilized into a new standard. That pattern is consistent with CHASE's endgame. The common playbook converges, and differentiation scatters into a long tail of unrepeated attempts.
...
<AccordionGroup>
  <Accordion title="What did the CHASE and Beyond the Vacuum papers predict?">
    CHASE (arXiv:2608.30466) simulated creators repeatedly optimizing content against an LLM ranking signal and found the ecosystem homogenizes: documents converge on similar features and the marginal gain of optimizing shrinks. Beyond the Vacuum (arXiv:2608.27631) formalized GEO as a competitor-aware problem: the value of any rewriting strategy depends on how many competitors have already adopted it. Our data shows both dynamics operating in live ChatGPT recommendation categories.
  </Accordion>
